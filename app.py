...
st.sidebar.image("https://your-logo-url.com/logo.png", width=120)
st.markdown("""
<div style="text-align: center;">
    <h1 style="color: red;">🧠 GenAI Research Summarizer</h1>
    <p style="font-size: 1.1em;">Upload your document and let AI help you understand, summarize, and test your knowledge.</p>
</div>
""", unsafe_allow_html=True)

st.title("(●'◡'●) GenAI Research Summarization")
st.markdown("""
Welcome to the GenAI Research Assistant! Upload a document and interact with it in multiple ways:
1. Summarize  summary
2. Question🙋‍♂️ From the content
3. Use Challenge Mode😁.Test your understanding with challenge😎 questions
""")
...
# Challenge Mode
st.markdown("---")
st.subheader(" Challenge Mode: Test Your Understanding")

if not st.session_state.document_text:
    st.info("In order to use Challenge Mode, please upload a document first.")
else:
    # Generate Questions Button
    if st.button("^_^ Generate Challenge Questions", use_container_width=True):
        with st.spinner("(❁´◡`❁)Let me create Some challenging question for your practice"):
            try:
                st.session_state.questions = generate_questions(st.session_state.document_text)
                st.session_state.show_questions = True
                st.session_state.show_results = False
                st.session_state.user_answers = {}
                st.success(" Challenge (O_O) questions generated!")
                st.rerun()
            except Exception as e:
